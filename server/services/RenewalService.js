...
export class RenewalService {
  constructor(configManager, broadcast) {
    ...
    this.lastLogs = new Map(); // id -> { message, time } for deduplication
  ...
  log(type, message, renewalId = null) {
    // 简单的日志去重：如果同一续期ID在 5秒内输出完全相同的日志，则忽略
    if (renewalId) {
      const lastLog = this.lastLogs.get(renewalId);
      const now = Date.now();
      if (lastLog && lastLog.message === message && (now - lastLog.time < 5000)) {
        return; // Skip duplicate log
      }
      this.lastLogs.set(renewalId, { message, time: now });
    }
